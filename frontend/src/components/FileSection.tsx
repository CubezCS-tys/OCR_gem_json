...
export default function FileSection({
  ...
  return (
    <div className="bg-white rounded-2xl border border-parchment-dark p-6 shadow-sm">
      {/* File header */}
      <div className="flex items-center gap-3 mb-6 pb-5 border-b border-parchment-dark">
        <div className="w-10 h-10 bg-ember/10 rounded-xl flex items-center justify-center flex-shrink-0">
          <svg className="w-5 h-5 text-ember" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
        ...
        <button
          onClick={onRemove}
          className="p-1.5 rounded-lg hover:bg-parchment transition-colors text-gray-400 hover:text-ink"
        >
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {/* Free */}
        <div className="border border-parchment-dark rounded-xl p-5 flex flex-col bg-parchment/40">
          <div className="flex items-center gap-2 mb-2">
            <span className="px-2 py-0.5 rounded-full bg-ink/10 text-ink text-xs font-semibold uppercase tracking-wide">Free</span>
            <h3 className="font-semibold text-ink">Searchable PDF</h3>
          </div>
          ...
          <ul className="text-sm text-gray-600 space-y-1.5 mb-4">
            {['Text search & copy', 'No sign-up needed', `Up to ${freeMaxPages} pages`].map((f) => (
              <li key={f} className="flex items-center gap-2">
                <svg className="w-3.5 h-3.5 text-green-500 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>
                {f}
              </li>
            ))}
          </ul>
          ...
          <button
            ...
            className="mt-auto w-full py-2.5 rounded-xl bg-ink hover:bg-ink-light disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold text-sm transition-colors"
          >
        ...
        {/* Pro */}
        <div className="border-2 border-ember/40 rounded-xl p-5 flex flex-col bg-ember-light/20">
          <div className="flex items-center gap-2 mb-2">
            <span className="px-2 py-0.5 rounded-full bg-ember-light text-ember-muted text-xs font-semibold uppercase tracking-wide">Pro</span>
            <h3 className="font-semibold text-ink">All Formats</h3>
          </div>
            ...
            {FORMAT_OPTIONS.map((opt) => (
              <label
                ...
                className={`fmt-chip inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-xs font-medium cursor-pointer transition-all select-none ${
                  checked[opt.value]
                    ? 'border-ember bg-ember-light text-ember-muted'
          ...
          <p className="text-xs text-gray-400 mb-3">
            This document: <strong className="text-ink">{pageCount} pages</strong>
          </p>

          <button
            disabled={uploading}
            onClick={handleProClick}
            className="mt-auto w-full py-2.5 rounded-xl bg-ember hover:bg-ember-dark disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold text-sm transition-colors"
          >
            {uploading ? 'Uploading…' : 'Convert — Pro'}
          </button>
          <p className="text-xs text-center mt-2 text-ember-muted">{proNote}</p>
        </div>
      </div>
    </div>
  );
